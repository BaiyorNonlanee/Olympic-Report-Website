...
import { defineProps, ref, computed, watchEffect, defineEmits } from 'vue'
...
const canEdit = computed(() => {
  return authStore.isAdmin || authStore.isMasterAdmin
})
</script>
...
<template>
  <div class="relative">
    ...
    <div
      v-if="country.ownSports && country.ownSports.length > 0"
    ...
    >
      ...
      >
        ...
        <tbody>
          <tr
            v-for="(sport, index) in editedSports"
            :key="sport.id"
          ...
          >
            <td class="px-2 py-2">
              <div v-if="isEditing">
                <input
                  v-model="sport.sportName"
                  class="border-2 border-gray-400 p-2 rounded w-full sm:w-[180px]"
                />
            ...
            <td class="px-2 py-2">
              <div v-if="isEditing">
                <InputTextSport
                  v-model="sport.gold_medals"
                  placeholder="Gold Medals"
                  :error="goldMedalsError"
                />
              </div>
              <div v-else>
                {{ sport.gold_medals }}
            ...
            <td class="px-2 py-2">
              <div v-if="isEditing">
                <InputTextSport
                  v-model="sport.silver_medals"
                  placeholder="Silver Medals"
                  :error="silverMedalsError"
                />
              </div>
              <div v-else>
                {{ sport.silver_medals }}
            ...
            <td class="px-2 py-2">
              <div v-if="isEditing">
                <InputTextSport
                  v-model="sport.bronze_medals"
                  placeholder="Bronze Medals"
                  :error="bronzeMedalsError"
                />
              </div>
              <div v-else>
                {{ sport.bronze_medals }}
      ...
      <div class="mt-4 flex justify-end w-full max-w-screen-lg">
        <button
          ...
          @click="toggleEdit"
          class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-full"
        >
          Edit
        </button>
        <button
          v-if="isEditing && canEdit"
          @click="submitChanges"
          class="ml-2 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-full"
        >
          Submit
        </button>
      </div>
