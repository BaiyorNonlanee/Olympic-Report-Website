...
watchEffect(() => {
  ...
  console.log(`Country: ${country.countryName}, Gold: ${country.sport.gold_medals}`);
});
...
const calculateAndSortCountries = () => {
  allCountries.value.forEach(country => {
    console.log(`Country: ${country.countryName}, Gold: ${country.sport.gold_medals}`); // ตรวจสอบค่าที่เป็น gold
  });

  // จัดเรียงประเทศตามจำนวนเหรียญทอง
  allCountries.value.sort((a, b) => b.sport.gold_medals - a.sport.gold_medals);
};
