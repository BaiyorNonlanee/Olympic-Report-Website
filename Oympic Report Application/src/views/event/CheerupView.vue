...
const addComment = () => {
  if (newComment.value.trim()) {
    ...
    console.log(userProfileImage[0])
    if (userProfileImage.length != 0) {
      ...
      commentsStore.addComment(props.country.countryName, newComment.value, username, userProfileImage[0])
      newComment.value = ''
...
<template>
  <div v-if="props.country" class="gap-4 h-50 px-4 py-6">
    ...
    <div class="w-95%">
      ...
      >
        <img :src="comment.images" alt="Profile" class="w-10 h-10 rounded-full mr-3" />
        <div>
